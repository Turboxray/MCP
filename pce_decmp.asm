...
;//
;// Calling parameters:
;// (stream) source address = REG X:Y
;// (stream) source bank = $FD
;// destination address = $FE/FF
;//
;// 
;// 
;// Header: 
...
;// History:
;//
;//   None. Starting over. Also, except for identifaction BYTEs, header isn't used.
;//
  ...
  ;processor Huc6280
        ...
        ;// zeropage usage
;LZPOS  EQU $209e   ; 2 ZeroPage temporaries
;bitstr EQU $20fb   ; 1 temporary (does not need to be ZP)

;puDECMP_ADDR EQU $c000
;puDECMP_BANK EQU $01
  ...
  .code
        ...
        ; $FF = HI of dest address, $FE = LO of dest address.
...
.8
    ...
    sta _esc  ; starting escape

    jsr getbyt  ; read startAddr
    lda <LZDEST ;//<$fe   ;// changed startAddr to ZP parameter
    sta <OUTPOS
    jsr getbyt
    lda <LZDEST+1 ;//<$ff
    sta <OUTPOS+1
    ...
    sec
    sbc _escB1
    sta _noesc  ; 8-escBits

    jsr getbyt
    sta _mg ; maxGamma + 1
    lda #9
    sec
    sbc _mg ; 8 - maxGamma == (8 + 1) - (maxGamma + 1)
    sta _longrle
    jsr getbyt
    sta _mg1  ; (1<<maxGamma)
    asl a
...
.1        ; setup bit store - $80 means empty
  ...
  newesc: 
    ldy _esc  ; remember the old code (top bits for escaped byte)
  escB0:  
    ldx _escB0 ;//#2    ; ** PARAMETER  0..8
  ...
  mg1:
    cmp _mg1  ;#64    ; ** PARAMETER 63-64 -> C clear, 64-64 -> C set..
    bcc chrcode ; short RLE, get bytecode

  longrle:
    ldx _longrle  ;#2   ; ** PARAMETER  111111xxxxxx
...
.1  
  ...
  lz77:
    jsr getval  ; X = 0
  mg21:
    cmp _mg21 ;#127 ; ** PARAMETER  Clears carry (is maximum value)
    bne noeof
  ...
  elzpb:
    ldx _elzpb  ;#0   ; ** PARAMETER (more bits to get)
...
.1
    ...
    inx
  mg:
    cpx _mg ;#7   ; ** PARAMETER unary code maximum length + 1
    bne gv0
